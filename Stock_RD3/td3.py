...
class ReinforcementLearner:
    ...
    def update_networks(self,
        ...
        # 배치 학습 데이터 생성
        x, acts, critic_target = self.get_batch(
            batch_size, delayed_reward, discount_factor)
        if len(x) > 0:
            loss = 0
            loss += self.critic.train(x, acts, critic_target)
            if self._update_step % self._target_update_interval == 0:
                # update actor
                loss +=  self.actor.train(x, self.critic.model)
            ...
            while True:
                ...
                if self.actor is not None:
                    pred_policy = self.actor.predict(
                        list(q_sample))
                    pred_target_policy =self.actor.target_predict(list(q_sample))
...
class TD3(ReinforcementLearner):
    ...
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        ...
        for i in range(q1_vals.shape[0]):
            critic_target[i] = discount_factor * q_vals[i] + rewards[i]



        return self.memory_sample, self.memory_action, critic_target
