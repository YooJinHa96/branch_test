...
class Agent:
    ...
    def decide_my_action(self, pred):
        my_action = np.argmax(pred)
        pred_con = pred.flatten()
        confidence = pred_con[my_action]
        ...
        my_unit = self.min_trading_unit + added_traiding

        return my_action, confidence, my_unit
