...
class ReinforcementLearner:
    ...
    def __init__(self, rl_method='rl', stock_code=None,
        ...
        self.sac = SAC(state_dim=self.num_features, action_dim=self.agent_NUM_ACTIONS, lr=self.lr , discount_factor=0.9, tau=self.tau)
    ...
    def run(
        ...
        for epoch in range(num_epoches):
            ...
            while True:
                ...
                pred_policy = self.sac.select_action(sample)
                ...
                if immediate_reward > 0.01 or immediate_reward < -0.01:
                    self.sac.store(sample, action, immediate_reward, next_sample)
               ...
               # self.memory_target_action.append(target_action)
                ...
                if learning and self.sac.num_transition> 10:
                    self.sac.update()
...
class TD3(ReinforcementLearner):
    ...
    def train(self, batch_size, delayed_reward, discount_factor):
        ...
        q1_values = self.critic.target_predict(next_samples, self.actor.target_predict(next_samples))
        q2_values = self.critic.target_predict2(next_samples, self.actor.target_predict(next_samples))


        # Compute critic target
        q_values = np.min(np.vstack([q1_values.transpose(), q2_values.transpose()]), axis=0)
        ...
        #print(actions)
        if self._update_step % self._target_update_interval == 0: # delayed policy upgrade
